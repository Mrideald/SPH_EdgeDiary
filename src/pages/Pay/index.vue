...
export default {
  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src="${url}"/>`, "请微信支付", {
        ...
        //messagebox显示右上角的叉叉
        showClose: false,
       //弹窗关闭之前 第一个参数是点击的哪一个 是取消type就是cancel 确认就是comfirm done是关闭弹窗
        beforeClose:(type,instance,done)=>{
          if(type=='cancel'){
            ...
            //如果是comfirm 判断是否真的支付成攻
            //if(this.code==200){   //不花钱 直接跳转
               //清除定时器
               clearInterval(this.timer);
              this.timer=null;
              done();
  ...
  mounted() {
    this.getPayInfo(); //不要在mounted写async await 会影响生命周期函数触发时间
  },
};
</script>
