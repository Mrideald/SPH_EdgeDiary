    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- 如果服务器数据还没回来就默认显示空数组【】，如果是空数组的话后面就不能.imgurl了，所以数据没回来的话就要加个默认空对象{} -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                ...
                :key="skuSaleAttrValue.saleAttrId"
              >
                <dt class="title">{{ skuSaleAttrValue.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: skuSaleAttrValue.saleAttrId == 1 }"
                  v-for="skuSaleAttrValue in skuInfo.skuSaleAttrValueList"
                  :key="skuSaleAttrValue.id"
                  @click="
                    changeActive(skuSaleAttrValue, skuInfo.skuSaleAttrValueList)
                  "
                >
...
export default {
  ...
  computed: {
    ...
    ...mapActions(["AddOrUpdateShopCart"]),
  },
  methods: {
    //产品售卖属性值变为高亮
    changeActive(saleAttrValue, arr) {
      ...
      //点击的那个售卖属性值为高亮
      saleAttrValue.saleAttrId = "1";
    },
    changeSkuNum(event) {
      //让用户输入的数据*1 若是非法 乘以1后会变为NaN
    ...
    async addShopCar() {
      ...
      try {
        ...
        //3：在路由跳转的时候需要将产品的信息带给下一级路由组件
        //一些简单的数据skuNum，通过query形式给路由组件传递过去
        //产品信息的数据【比较复杂:skuInfo】，通过会话存储（不持久，会话结束后数据消失）
        //本地存储|会话存储，一般存储的是字符串,可以通过JSON转换对象
        //用会话存储
